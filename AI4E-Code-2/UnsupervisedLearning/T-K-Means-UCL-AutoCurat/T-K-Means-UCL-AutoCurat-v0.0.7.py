...
# Chopping off the output layer; using the penultimate layer for feature extraction.
...
def convert_imgs_to_feat(images, num_pics, num_features):
    ...
    for img_i in range(num_pics):
        ...
        # Convert to features
        input_tensor = preprocess(images[img_i])
        input_batch = input_tensor.unsqueeze(0) # create a mini-batch as expected by the model
        output = vgg16_model(input_batch)
        # Store in data
        data[img_i,:] = output.detach().numpy()
        #index = torch.tensor([img_i])
        #data.index_copy_(0, index, output)
        #del output
...
num_iters = 50
num_ks = [5, 10, 20] # [5,10,20,30] # [5,10,15,20]
