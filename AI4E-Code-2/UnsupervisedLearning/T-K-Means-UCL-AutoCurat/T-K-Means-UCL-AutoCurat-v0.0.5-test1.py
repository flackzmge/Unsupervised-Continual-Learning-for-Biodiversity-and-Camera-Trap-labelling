...
# v0.0.5 - Add adaptive thresholds.
...
num_iters = 50
num_clusters = 30 # 5 # 10 # 4  # initial number of clusters
...
def get_closest_centroid(x, centroids):
    ...
    closest_centroid_dist = np.min(dist, axis=1)
    
    return closest_centroid_index, closest_centroid_dist
...
# --- Parameters ---
#new_cluster_thresh = 275902734.0
new_cluster_thresh = sys.maxsize
#cluster_merge_thresh = 55902734.0
cluster_merge_thresh = 0
# ------------------
...
for n in range(num_iters):
    ...
    assigned_centroids, closest_centroid_dist  = get_closest_centroid(data[:, None, :], centroids[None,:, :])    
    
    # Compute new centroids
    for c in range(centroids.shape[0]):
        # Get data points belonging to each cluster
        new_clusters = closest_centroid_dist > new_cluster_thresh
        # cluster_members = data[assigned_centroids == c]
        cluster_members = data[((assigned_centroids == c) * np.invert(new_clusters))]
    ...
    # Temporary - check number of clusters
    num_new_clusters = np.sum(new_clusters)
    if num_new_clusters > 0:
        print('Memory warning!')
        print('num_new_clusters is {0}'.format(num_new_clusters))
        ans = input('Do you want to continue? (yes/no) ')
        if ans == 'no':
            quit()

    # --- The code below has not been fully tested. It is raw and probbly buggy. ------

    # Extract new centroids
    new_centroids = data[new_clusters]
    # Append to centroid matrix
    centroids = np.append(centroids, new_centroids, axis=0)
    num_clusters = centroids.shape[0]
    print('num_clusters: {0}'.format(num_clusters))
    
    # Fuse centroids that are too similar
    # Compute distance matrix
    a_dist_mat = distance_matrix(centroids, centroids, 2) ** 2
    np.fill_diagonal(a_dist_mat, sys.maxsize)
    already_found = []
    fused_centroids = []
    list_unique = []
    # Search pairs to fuse
    for c1i in range(num_clusters):
        if c1i not in already_found:
            # Compute min
            distance_row = a_dist_mat[c1i,:]
            min_index = np.argmin(distance_row)
            min_dist = distance_row[min_index]
            # Check threshold
            if min_dist < cluster_merge_thresh:
                fused_centroids.append((c1i, min_index))
                a_dist_mat[:,min_index] = sys.maxsize    
                already_found.append(min_index)
            else:
                list_unique.append(c1i)              
            a_dist_mat[:,c1i] = sys.maxsize
            already_found.append(c1i)
    # Re-organize new centroids
    # Get unique 
    list_new_centroids = []
    for unique_centroid_i in list_unique:
        a_centroid = centroids[unique_centroid_i,:]
        list_new_centroids.append(a_centroid)
    # Fuse pairs and add
    for fuse_pair in fused_centroids:
        centroid_1 = centroids[fuse_pair[0],:]
        centroid_2 = centroids[fuse_pair[1],:]
        mean_centroid = (centroid_1 + centroid_2) / 2
        list_new_centroids.append(mean_centroid)

    # --- The code above has not been fully tested. It is raw and probbly buggy. ------

    centroids = np.asarray(list_new_centroids)
    num_clusters = centroids.shape[0]
