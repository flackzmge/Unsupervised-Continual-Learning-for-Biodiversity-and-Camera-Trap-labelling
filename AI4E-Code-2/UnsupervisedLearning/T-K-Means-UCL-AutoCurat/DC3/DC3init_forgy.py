...
def macqueen(data, k, metric='e', passes=1):
    nX, mX = data.shape
...
def comp_batch(source_path, boundaries_path_file, scale_fac, centroids):
    ...
    #num_ks = [27, 28, 29, 30]
    num_ks = [5, 10, 10, 15]
    init_type = 3 # 1 = random; 2 = k-means++, 3 = forgy
    ...
    for num_clusters in num_ks: # Design choice 6 (DC6). Different search procedures for optimal k (e.g. grid search is a baseline).
        ...
        if init_type == 1:
            ...
            centroids == forgy(data,num_clusters)
        elif init_type == 5:
            centroids == macqueen(data, num_clusters)
        elif init_type == 6:
            centroids == maximin(data, num_clusters)
        elif init_type == 7:
            centroids == var_part(data, num_clusters)
