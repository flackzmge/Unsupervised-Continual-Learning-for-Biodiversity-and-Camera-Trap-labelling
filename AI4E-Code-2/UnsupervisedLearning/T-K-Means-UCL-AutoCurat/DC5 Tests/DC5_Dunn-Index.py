...
def dunn_index(X, labels):
    ...
    The Dunn index is defined as the ratio of the minimum and maximum
    cluster separation to the average cluster separation.
    ...
    References
    ----------
    .. [1] `V. Dunn, 1979. "The calculation of Fuzzy Numbers and Their
       Applications to Cluster Analysis". IEEE Transactions
    ...
    min_dist, max_dist = np.inf, 0.0
    mean = np.mean(X, axis=0)
    for k in range(n_labels):
        cluster_k = X[labels == k]
        dist = np.mean(chebyshev(cluster_k, cluster_k))
        if dist < min_dist:
            min_dist = dist
        if dist > max_dist:
            max_dist = dist

    return (max_dist - min_dist) / (max_dist + min_dist)
# === Compute batch ==================================
...
def comp_batch(source_path, boundaries_path_file, scale_fac, centroids):
    ...
    for num_clusters in num_ks: # Design choice 6 (DC6). Different search procedures for optimal k (e.g. grid search is a baseline).
        ...
        di_score = dunn_index(data, assigned_centroids)
        ...
        print('Dunn index for k = {0}: {1}'.format(num_clusters, di_score))
        db_score_list.append(db_score)
        ch_score_list.append(ch_score)
        di_score_list.append(di_score)
    ...
    # Select best k
    best_k_i = np.argmin(di_score_list)
