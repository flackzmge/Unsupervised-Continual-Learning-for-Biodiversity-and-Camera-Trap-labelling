...
def cosine_similarity101(vector_1, vector_2):
    #initialize vector sums
    sumv1, sumv2, sumv1v2 = 0, 0, 0
    #range(len(np.shape(vector_1)))
...
def cosine_similarity102(vector_1, vector_2):
    #initialize vector sums
    sumv1, sumv2, sumv1v2 = 0, 0, 0
    #range(len(np.shape(vector_1)))
...
def comp_batch(source_path, boundaries_path_file, scale_fac, centroids):
    ...
    while batchCount > 0:
        ThisBatch = all_assigned_centroids[batchCount - 1]
        if cosine_similarity101(ThisBatch, assigned_centroids) > 0.8:
            # assigned_centroids.append(all_assigned_centroids[batch][batchdata])w
            assigned_centroids = np.append(assigned_centroids, ThisBatch)
...
''' 
    hello.append(assigned_centroids)
    if b_i == 0:
        # First batch
        pass
    else:
        yolo = cosine_similarity101(hello[b_i - 1],hello[b_i])
        print(yolo)
        print('hi')  '''
